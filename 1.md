...
Para el antirrebote se implementa un módulo que utiliza una máquina de estados finitos para monitorizar el estado del botón, junto con un contador para medir el tiempo de debounce. Filtra los posibles rebotes tanto cuando el botón es presionado como cuando es soltado, asegurando que las señales de salida (db_level y db_tick) sean estables y confiables. Además, ajusta el tiempo de debounce en función del modo seleccionado (normal o largo) dependiendo del boton.
...
## (BORRAR UNA VEZ HECHO) MANUEL AQUI TOCA COLOCAR LA CAJA NEGRA CORRESPONDIENTE AL FUNCIONAMIENTO DE LA PANTALLA (SOLO UNA IMGAGEN) Y SE COLOCAN SECCION O SECCIONES DEPENDIENDO DE QUE MAS TIENE LA PANTALLA POR EJEMPLO DESPUES DE LO QUE YA ESTA ESCRITO, HAY UN ESPACIO PARA UNA SECCION QUE SE VA A LLAMAR "FMS DE LA PANTALLA" EN DONDE EXPLICA COMO FUNCIONA Y QUE TIENE Y COLOCA UN DIAGRAMA DE ESE FMS AL INICIO DE ESA SECCION
